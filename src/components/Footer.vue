<template>
    <div>
        ...
        <footer id="one" v-else-if="!footer && directive">
            {{ directive }} 
        </footer>
         <footer v-else>
             directive: <span>v-else</span> 
        </footer>
        ...
        <button @click="changeClass($event)" v-show="!footer && !directive">Change Class!</button>
    </div>
</template>
...
export default {
    ...
    methods: {
        changeClass($event) {
            console.log($event)
            this.conditionalClass = !this.conditionalClass
        }
    }
}
</script>
